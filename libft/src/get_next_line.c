...
/*   Updated: 2023/04/13 19:11:58 by gehovhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_move_start(char	*start)
{
	...
	i += (start[i] == '\n');
	new_buff = (char *)malloc(1 + ft_gnl_strlen(start) - i);
	if (!new_buff)
		return (NULL);
	j = 0;
	while (start[i + j])
...
char	*get_next_line(int fd)
{
	...
	if (!tmp)
		return (NULL);
	while (!(ft_gnl_strchr(start_str[fd], '\n')) && fd_read != 0)
	{
		...
		tmp[fd_read] = '\0';
		start_str[fd] = ft_gnl_strjoin(start_str[fd], tmp);
	}
	free(tmp);
